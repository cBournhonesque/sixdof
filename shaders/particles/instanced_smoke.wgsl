...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let uv_smoke = vec2<f32>(uv.x + time * 0.1, uv.y + time * -0.1);
    let uv_smoke_final = vec2<f32>(
        uv_smoke.x + cos(uv_smoke.x * 1.0 + time * 1.2) * 0.1,
        uv_smoke.y + sin(uv_smoke.y * 1.0 + time * 1.2) * 0.1
    );

    let uv_thirded = in.uv * 3.0 - 1.5;
    let uv_core = vec2<f32>(
        uv_thirded.x + cos(uv_thirded.x * 1.0 + time * 5.2) * 0.1,
        uv_thirded.y + sin(uv_thirded.y * 1.0 + time * 5.2) * 0.1
    );
    let core = soft_circle(uv_core / 2.0, 0.5);
...
fn soft_circle(position: vec2<f32>, radius: f32) -> f32 {
    return 1.0 - smoothstep(-radius, radius, length(position));
}
