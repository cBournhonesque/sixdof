...
pub(crate) fn log_plugin() -> LogPlugin {
    LogPlugin {
        level: Level::INFO,
        filter: "wgpu=error,bevy_render=info,bevy_ecs=warn,bevy_time=warn,lightyear::client::prediction::rollback=debug,lightyear::client::prediction::prespawn=debug,lightyear::server::prediction=debug".to_string(),
        ..default()
    }
}
